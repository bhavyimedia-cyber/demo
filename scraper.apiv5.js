...
async function scrapeTrademarks({ keyword, tmClass, searchType, matchType }) {
    ...
    try {
        ...
        // ── Wait for any result/error indicator ───────────────────────────────
        // If this times out it means the page itself failed to respond.
        // Throw a ScrapingError so the caller can apply retry logic instead of
        // silently returning an empty result set.
        const resultAppeared = await page.waitForSelector(RESULT_SELECTORS, { timeout: 100000 })
            .then(() => true)
            .catch(() => false);

        if (!resultAppeared) {
            throw new ScrapingError(
                "Timed out waiting for search results — page did not respond in time.",
                "RESULT_TIMEOUT"
            );
        }
        ...
        // Only return empty results when the SITE ITSELF confirms no records.
        // Any other empty-looking state should throw so it can be retried.
        ...
        // If we reach here without the table, the page is in an unexpected state.
        // Throw instead of returning empty so the caller can retry.
        const tableEl = await page.$("#ContentPlaceHolder1_MGVSearchResult").catch(() => null);

        if (!tableEl) {
            throw new ScrapingError(
                "Results table not found despite page load completing — page may not have rendered correctly.",
        ...
        if (rowCount < 2) {
            throw new ScrapingError(
                "Results table found but contains no data rows — page may not have loaded correctly.",
                "RESULT_TABLE_EMPTY"
            );
        }
        ...
        // ── Guard: normalization wiped all rows ───────────────────────────────
        // The table had rows but parsing produced nothing — likely a layout change
        // on the site. Throw so the caller knows this isn't a genuine "no data".
        if (cleanedData.length === 0) {
            throw new ScrapingError(
                `Table had ${rowCount - 1} row(s) but all were empty after normalization — possible site layout change.`,
                "EXTRACTION_EMPTY"
            );
        }
